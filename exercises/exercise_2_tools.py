...
def check_statute_of_limitations(case_type: str) -> str:
    ...
    limits = {
        ...
        "breach": "4 năm (UCC § 2-725)",
        "hợp đồng": "4 năm theo quy định chung về tranh chấp hợp đồng",
        "tort": "2-3 năm tùy bang",
        "property": "5 năm",
        "labor": "1 năm đối với một số tranh chấp lao động cá nhân",
        "lao động": "1 năm đối với một số tranh chấp lao động cá nhân",
    }

    case_type_lower = case_type.lower()
    for keyword, limit in limits.items():
        if keyword in case_type_lower:
            return f"Thời hiệu khởi kiện cho '{case_type}': {limit}."
    return f"Chưa có dữ liệu thời hiệu cho '{case_type}'."
